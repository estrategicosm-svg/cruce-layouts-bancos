...
from exportadores.reportes import registros_a_dataframe, resumen_a_dataframe
...
class SATConciliatorApp:
    ...
    def _sidebar(self) -> None:
        with st.sidebar:
            ...
            cedula_file = st.file_uploader("Cedula SAT Excel", type=["xlsx", "xls"])
            ...
            if banco_file and banco_file.name.lower().endswith('.pdf'):
                ...
                if not validacion.get("valido"):
                    st.error("⚠️ Advertencias detectadas en la extracción:")
            ...
            if st.button("Procesar archivos", type="primary", use_container_width=True):
                if not cedula_file or not banco_file or not xml_zip:
                    st.error("Carga la cedula, el estado de cuenta y el ZIP de XML.")
                    ...
                    self._procesar(cedula_file, banco_file, xml_zip, Decimal(str(tolerancia)), dias_tolerancia)
    ...
    def _main(self) -> None:
        ...
        col1.metric("Cedula", len(st.session_state.cedula or []))
        col2.metric("XML CFDI", len(st.session_state.cfdis or []))
        col3.metric("Banco", len(st.session_state.movimientos or []))
        col4.metric("Conciliados", resultado.conciliados if resultado else 0)
    ...
    def _mostrar_resultado(self, resultado) -> None:
        ...
        tabs = st.tabs(["Resumen", "Registros", "Diferencias", "Sin XML", "Sin banco", "Validacion"])
        with tabs[0]:
        ...
        with tabs[5]:
            errores = st.session_state.errores + resultado.errores
            if errores:
    ...
    def _procesar(self, cedula_file, banco_file, xml_zip, tolerancia: Decimal, dias_tolerancia: int) -> None:
        with st.spinner("Procesando conciliacion..."):
            ...
            cfdis = xml_parser.parsear_zip(xml_zip.getvalue())
            ...
            datos_excel = {
                ...
                "ESTADOS_CUENTA": self._movimientos_df(movimientos),
                "DIFERENCIAS": registros_df[registros_df["Estatus"].astype(str).str.contains("DIFERENCIA|FUERA|MONEDA|DUPLICADO", na=False)],
                "SIN_XML": registros_df[registros_df["Estatus"] == "SIN XML"],
                "SIN_BANCO": registros_df[registros_df["Estatus"] == "SIN BANCO"],
                "VALIDACION": pd.DataFrame({"Errores": cedula_parser.errores + banco_parser.errores + xml_parser.errores + resultado.errores}),
            }
            ...
            st.session_state.errores = cedula_parser.errores + banco_parser.errores + xml_parser.errores
    ...
    def _movimientos_df(self, movimientos) -> pd.DataFrame:
        return pd.DataFrame(
            [
                {
                    "Banco": mov.banco,
